...
public class ConsistentHashingII {
    ...
    private static int shareSize;
    private HashSet<Integer> sharePoints = new HashSet<Integer>();
    private HashMap<Integer, List<Integer>> machineMap =
        new HashMap<Integer, List<Integer>>();
    private HashMap<Integer, Integer> microShard =
        new HashMap<Integer, Integer>();

    public ConsistentHashingII(){
    }
...
//    }
    ...
    public List<Integer> addMachine(int machine_id) {
        // Write your code here
        ArrayList<Integer> machinePoints = new ArrayList<Integer>();
        for (int i = 0; i < shareSize; i++) {
            int point = getRandom();
            machinePoints.add(point);
            microShard.put(point, machine_id);
        }
        machineMap.put(machine_id, machinePoints);
        return machinePoints;
    }
    // 如何产生随机数，如果证明等概论，如果抽样。。。。。。
    private int getRandom() {
        int mutiply = 1000;
        int num = (int)((Math.random() *  mutiply) % capacity);
        while (sharePoints.contains(num)) {
            num = (int)((Math.random() * mutiply) % capacity);
        }
        sharePoints.add(num);
        return num;
    }
    ...
    public int getMachineIdByHashCode(int hashcode) {
        // Write your code here
        while (!microShard.containsKey(hashcode)) {
            hashcode = (hashcode + 1) % capacity;
        }
        return microShard.get(hashcode);
    }
    public static void main(String[] args) {
    	ConsistentHashingII ch = ConsistentHashingII.create(100, 3);
    	ch.addMachine(1);
    	ch.getMachineIdByHashCode(4);
    	ch.addMachine(2);
    	ch.getMachineIdByHashCode(61);
    	ch.getMachineIdByHashCode(91);
    }
}
