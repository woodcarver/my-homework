...
这种方法的缺点是，当数据继续增加，我们需要增加数据库服务器，将 n 变为 n+1 时，
几乎所有的数据都要移动，这就造成了不 consistent。为了减少这种 naive 的 hash方法(%n) 带来的缺陷，
...
public class ConsistentHashing {
    public List<List<Integer>> consistentHashing(int n) {
        ...
        for (int i = 2; i <= n; i++) {
            reHashing(hashList, i);
        }
        return hashList;
    }
    private void reHashing(List<List<Integer>> hashList, int segemntName) {
        if (hashList == null) {
            return;
        }
        List<Integer> maxList = null;
        int maxGap = 0;
        for (int i = 0, len = hashList.size(); i < len; i++) {
            int gap = hashList.get(i).get(1) - hashList.get(i).get(0);
            if (gap > maxGap) {
                maxGap = gap;
                maxList = hashList.get(i);
            }
        }
        if (maxList == null) {
            return;
        ...
        // middle忘记加上base -- maxList.get(0)
        int middle = maxList.get(0) + (maxList.get(1) - maxList.get(0)) / 2;
        List<Integer> newSegment = new ArrayList<Integer>();
        newSegment.add(middle + 1);
        newSegment.add(maxList.get(1));
        newSegment.add(segemntName);
        hashList.add(newSegment);

        maxList.set(1, middle);
    }
}
